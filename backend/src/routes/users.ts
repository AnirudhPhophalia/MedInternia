...
import {
  toggleBookmark,
  getSavedItems,
} from "../controllers/userBookmarksController";
...
const resumeUpload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if (!isAllowedResumeUpload(file.originalname, file.mimetype)) {
      cb(
        new Error(
          "Only PDF, DOCX, and TXT files are allowed for resume upload",
        ),
      );
      return;
    }
...
router.patch(
  "/:userId/verify",
  authenticate,
  requirePermission("profile:verify"),
  verifyDoctor,
);

// Grant contributor badge
router.post(
  "/:userId/grant-contributor",
  authenticate,
...
router.post(
  "/:internId/award-points",
  authenticate,
  requirePermission("user:award_points"),
  awardPointsToIntern,
);
