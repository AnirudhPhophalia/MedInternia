  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const pathsWithProgress = paths.map((path) => {
      const progress = userProgress.find(
        (p) => String(p.learningPath) === String(path._id),
      );
      return {
        ...path.toObject(),
...
export const getLearningPathById = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    let progress = null;
    if (req.user) {
      progress = await UserLearningPath.findOne({
        user: req.user._id,
        learningPath: id,
      });
    }
...
export const enrollInPath = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    if (!req.user) {
    ...
    const existingEnrollment = await UserLearningPath.findOne({
      user: req.user._id,
      learningPath: id,
    });
...
// Complete a step
export const completeStep = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    if (!req.user) {
      throw new AppError("User not authenticated", 401);
    }
    ...
    const progress = await UserLearningPath.findOne({
      user: req.user._id,
      learningPath: id,
    });
    if (!progress) {
