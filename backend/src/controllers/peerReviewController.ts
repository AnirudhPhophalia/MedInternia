...
export const submitPeerReview = async (req: AuthRequest, res: Response) => {
  try {
    const { revieweeId, caseId, commentId, rating, feedback, comments, tags } =
      req.body;
    ...
    try {
      ...
      const revieweeReviews = await PeerReview.find({
        reviewee: revieweeId,
      }).session(session);
      const totalRating = revieweeReviews.reduce(
...
export const getUserReviews = async (req: Request, res: Response) => {
  try {
    ...
    if (allReviews.length > 0) {
      ...
      averages.accuracy =
        allReviews.reduce((sum, r) => sum + r.feedback.accuracy, 0) /
        allReviews.length;
      averages.clarity =
        allReviews.reduce((sum, r) => sum + r.feedback.clarity, 0) /
        allReviews.length;
      averages.completeness =
        allReviews.reduce((sum, r) => sum + r.feedback.completeness, 0) /
        allReviews.length;
      averages.reasoning =
        allReviews.reduce((sum, r) => sum + r.feedback.reasoning, 0) /
        allReviews.length;
    }
