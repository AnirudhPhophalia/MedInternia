  ...
  async (req: AuthRequest, res: Response) => {
    ...
    if (req.query.specialization) {
      filter.specialization = {
        $regex: String(req.query.specialization),
        $options: "i",
      };
    }
    ...
    const [cases, total] = await Promise.all([
      Case.find(filter)
        .populate(
          "doctor",
          "firstName lastName specialization avatar medicalLicenseVerified",
        )
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit),
...
export const getCaseById = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    const caseDoc = await Case.findOne({
      ...
      .populate(
        "comments.author",
        "firstName lastName userType avatar medicalLicenseVerified",
      )
...
export const updateCase = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    if (
      (caseDoc as any).doctor?.toString() !== user._id!.toString() &&
      user.userType !== "admin"
    ) {
...
export const toggleLike = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      ...
      data: {
        likesCount: ((updatedCase as any).likes || []).length,
        hasLiked: !hasLiked,
      },
    });
  },
...
export const toggleStar = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    const hasStarred = savedCases.some(
      (id: any) => id.toString() === (caseDoc as any)._id?.toString(),
    );

    if (hasStarred) {
      await User.findByIdAndUpdate(user._id, {
        $pull: { savedCases: caseDoc._id },
      });
    } else {
      await User.findByIdAndUpdate(user._id, {
        $addToSet: { savedCases: caseDoc._id },
      });
    }
...
export const getStarredCases = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      success: true,
      data: { cases: (userDoc as any)?.savedCases || [] },
    });
  },
);
...
export const getLikedCases = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    const cases = await Case.find({ likes: user._id, ...baseFilter }).populate(
      "doctor",
...
export const addComment = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    const doctorId = doctorField?._id
      ? doctorField._id.toString()
      : doctorField?.toString();
...
export const toggleRepostPermission = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    if ((caseDoc as any).doctor?.toString() !== user._id!.toString()) {
      throw new AppError(
        "Only the case author can change repost permissions",
        403,
      );
    }
...
export const repostCase = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    if (!(originalCase as any).allowRepost) {
      throw new AppError(
        "This case cannot be reposted per author restrictions",
        400,
      );
    }
...
export const generateAISuggestions = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    if ((caseDoc as any).doctor?.toString() !== user._id!.toString()) {
      throw new AppError(
        "You can only generate AI suggestions for your own cases",
        403,
      );
    }
...
export const getCaseAISuggestions = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      success: true,
      data: { aiAnalysis: (caseDoc as any).aiAnalysis || null },
    });
  },
);
...
export const getCaseFollowUps = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      success: true,
      data: { followUps: (caseDoc as any).followUps || [] },
    });
  },
);
...
export const publishDueAICasePosts = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    return res.json({
      success: true,
      data: { count: published.length, cases: published },
    });
  },
);
...
export const likeComment = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    const likes =
      ((updatedCase?.comments as any)?.[0]?.likes as any[])?.length ?? 0;
...
export const rateComment = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    if (!rating || rating < 1 || rating > 5)
      throw new AppError("Rating bounds error", 400);

    const caseDoc = await Case.findById(getId(caseId), {
      comments: { $elemMatch: { _id: getId(commentId) } },
    });
    if (!caseDoc || !(caseDoc.comments as any)?.[0])
...
export const uploadAttachment = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    res
      .status(201)
      .json({ success: true, data: { url: uploadResult.secure_url, type } });
  },
);
...
export const getSimilarCases = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const caseDoc = await Case.findById(getId(req.params.id));
    if (!caseDoc) {
      throw new AppError("Case not found", 404);
