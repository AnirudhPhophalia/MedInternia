...
export const getCollectionById = async (
  ...
  try {
    ...
    await collection.populate({
      ...
      populate: {
        path: "doctor",
        select: "firstName lastName profilePicture specialization",
      },
    });
...
// @access  Private
export const addCaseToCollection = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    if (collection.cases.some((id: any) => id.toString() === caseId)) {
      res
        .status(400)
        .json({ success: false, message: "Case already in collection" });
      return;
    }
    ...
    res
      ...
      .json({
        success: true,
        data: collection,
        message: "Case added to collection",
      });
  } catch (error: any) {
...
// @access  Private
export const removeCaseFromCollection = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    collection.cases = collection.cases.filter(
      (id: any) => id.toString() !== req.params.caseId,
    );
    await collection.save();
    ...
    res
      ...
      .json({
        success: true,
        data: collection,
        message: "Case removed from collection",
      });
  } catch (error: any) {
