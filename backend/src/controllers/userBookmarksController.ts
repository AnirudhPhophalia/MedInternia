  ...
  try {
    ...
    if (!canAccessSavedItems(req, userId)) {
      return res
        ...
        .json({
          success: false,
          message: "Not authorized to modify saved items for this user",
        });
    }
    ...
    switch (itemType) {
      ...
      default:
        return res
          ...
          .json({
            success: false,
            message: "Invalid itemType. Must be case, job, or webinar.",
          });
    }
  ...
  try {
    ...
    if (!canAccessSavedItems(req, userId)) {
      return res
        ...
        .json({
          success: false,
          message: "Not authorized to view saved items for this user",
        });
    }
    const user = await User.findById(userId)
      .populate("savedCases")
