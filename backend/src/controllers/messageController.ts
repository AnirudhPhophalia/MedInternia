...
export const getConversations = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    const conversations = await Conversation.find({
      ...
      .populate(
        "participants",
        "firstName lastName profilePicture userType isVerifiedDoctor",
      )
      .sort({ updatedAt: -1 });
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    if (receiver.messagePrivacy === "verified_only") {
      ...
      if (!sender?.isVerifiedDoctor && !sender?.isVerified) {
        throw new AppError(
          "This user only accepts messages from verified users",
          403,
        );
      }
    }
    ...
    const preview =
      trimmedContent.length > PREVIEW_MAX_LENGTH
