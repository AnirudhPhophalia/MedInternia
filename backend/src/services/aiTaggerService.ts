...
function getLocalFallbackTags(
  ...
  if (
    textToAnalyze.includes("surgery") ||
    textToAnalyze.includes("operative")
  ) {
    tagsSet.add("surgery");
  }
  if (
    textToAnalyze.includes("pediatric") ||
    textToAnalyze.includes("child") ||
    textToAnalyze.includes("infant")
  ) {
    tagsSet.add("pediatrics");
  }
  if (
    textToAnalyze.includes("therapy") ||
    textToAnalyze.includes("medication") ||
    textToAnalyze.includes("drug")
  ) {
...
export async function analyzeCase(
  ...
  const localFallback = getLocalFallbackTags(
    title,
    description,
    specialization,
  );
  const fallbackResult: AITaggedOutput = {
    ...
    treatment:
      localFallback.treatment || "Further diagnostic workup recommended.",
  };

  if (!GEMINI_API_KEY) {
    console.log(
      "GEMINI_API_KEY not found. Using local clinical fallback for AI tagging.",
    );
...
Ensure the output is ONLY valid JSON. Do not include markdown code block formatting or any explanation outside the JSON.`;
    ...
    return {
      difficulty: matchedDifficulty.toLowerCase() as
    ...
    console.error(
      "Failed to analyze case via Gemini API, using default fallback:",
      error,
    );
    return fallbackResult;
  }
