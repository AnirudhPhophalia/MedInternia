  ...
  return jest.fn().mockImplementation((buffer) => {
    if (buffer.toString() === "mock-pdf-buffer") {
      return Promise.resolve({
        text: "Cardiology resident with 3 years of experience at Delhi Medical School. Skills: Surgery, Suturing, Python.",
      });
    }
  ...
  return {
    extractRawText: jest.fn().mockImplementation((options) => {
      if (options.buffer.toString() === "mock-docx-buffer") {
        return Promise.resolve({
          value:
            "Pediatrics specialist with 5 years experience. Medical school: Harvard School of Medicine. Skills: Patient Care, CPR.",
        });
      }
...
describe("Resume Parser Service", () => {
  describe("extractTextFromBuffer", () => {
    ...
    it("throws error for unsupported file types", async () => {
      ...
      await expect(
        extractTextFromBuffer(buf, "image/png", "resume.png"),
  ...
  describe("parseResumeText", () => {
    ...
    it("handles fallback bio construction when no summary is found", () => {
      ...
      expect(result.bio).toBe(
        "I am studied at Delhi School of Medicine with 2 years of clinical experience. Key areas of focus include Suturing, Research.",
      );
    });
