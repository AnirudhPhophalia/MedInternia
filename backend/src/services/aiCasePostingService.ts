...
export const AI_CASE_POSTING_INTERVALS = [
  "daily",
  "weekly",
  "biweekly",
  "monthly",
] as const;
export const AI_CASE_REVIEW_STATUSES = [
  "pending",
  "approved",
  "changes_requested",
  "rejected",
...
const normalizePatientGender = (
  value: unknown,
): GeneratedAICase["patientInfo"]["gender"] => {
...
export const buildAICaseSchedule = (
  body: AICaseScheduleInput,
): ValidatedAICaseSchedule => {
  ...
  const specialization =
    normalizeText(body.specialization) ??
    primaryPrediction?.specialty ??
    "General Medicine";
  const difficulty = normalizeDifficulty(body.difficulty);
  const learningObjective =
    normalizeText(body.learningObjective) ??
    `Practice triage reasoning for ${specialization}.`;
  const diagnosis =
    primaryPrediction?.condition ?? "Undifferentiated clinical presentation";
  ...
  const generatedCase: GeneratedAICase = {
    ...
    diagnosis,
    treatment:
      primaryPrediction?.recommendations.join("\n") ??
      "Clinician review required.",
    tags: Array.from(
      new Set(["ai-generated", "scheduled-case", specialization.toLowerCase()]),
    ),
    difficulty,
    specialization,
    aiSummary: [
