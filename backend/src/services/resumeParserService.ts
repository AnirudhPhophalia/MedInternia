...
export function parseResumeText(text: string): ExtractedResumeData {
  ...
  if (!medicalSchool) {
    const fallbackRegex =
      /([A-Za-z0-9\s,.-]+(?:University|College|School of Medicine|Medical School|Institute)[A-Za-z0-9\s,.-]*)/i;
  ...
  if (summaryStartIndex !== -1) {
    let summaryText = "";
    for (
      let i = summaryStartIndex;
      i < Math.min(summaryStartIndex + 4, lines.length);
      i++
    ) {
  ...
  if (!bio) {
    const schoolPart = medicalSchool
      ? `studied at ${medicalSchool}`
      : "a medical student";
    const expPart =
      experience > 0 ? ` with ${experience} years of clinical experience` : "";
    const skillsPart =
      skills.length > 0
        ? `. Key areas of focus include ${skills.slice(0, 4).join(", ")}`
        : "";
