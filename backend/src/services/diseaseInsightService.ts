...
export const validatePredictionInput = (body: unknown): PredictionInput => {
  ...
  return {
    ...
    age,
    gender:
      typeof payload.gender === "string" ? payload.gender.trim() : undefined,
    duration:
...
export const predictDiseaseInsights = (
  ...
  return diseaseProfiles
    .map((profile) => {
      const matchedSymptoms = Array.from(
        new Set(
          profile.symptoms.flatMap((symptom) =>
            symptomMatches(reportedSymptoms, symptom),
          ),
        ),
      );
      const redFlags = profile.redFlags.filter(
        (flag) => symptomMatches(reportedSymptoms, flag).length > 0,
      );
      const confidence = Math.min(
        0.95,
        Math.round(
          (matchedSymptoms.length / Math.max(profile.symptoms.length, 1)) * 100,
        ) / 100,
      );
      ...
      return {
        ...
        reasoning: [
          ...
          input.duration
            ? `Reported duration: ${input.duration}.`
