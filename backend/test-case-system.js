...
async function testCaseSystem() {
  ...
  const newCase = {
    ...
    description:
      "A 45-year-old male presents with chest pain and shortness of breath. ECG shows ST elevation in leads II, III, and aVF.",
    ...
    treatment:
      "Emergency PCI (percutaneous coronary intervention) performed. Patient stabilized.",
  ...
  const comment = {
    content:
      "Based on the presentation, I agree with the diagnosis. The inferior wall STEMI is clearly indicated by the ST elevation in leads II, III, and aVF. Quick intervention was crucial here. Great case for learning!",
  };
  ...
  const doctorComment = {
    content:
      "Excellent observation! This case highlights the importance of rapid ECG interpretation and immediate cardiac catheterization. The patient had 99% occlusion of the RCA (right coronary artery).",
  };
  ...
  await makeRequest(
    `${API_BASE}/cases?difficulty=advanced&specialization=cardiology`,
    {
      headers: {
        Authorization: `Bearer ${patientToken}`,
      },
  ...
  const updateData = {
    ...
    description:
      newCase.description +
      " UPDATE: Patient recovered well and was discharged after 3 days.",
  };
