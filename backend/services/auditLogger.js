...
const startAuditLogger = async () => {
  try {
    ...
    changeStream.on("change", async (change) => {
      try {
        ...
        if (change.comment) {
          try {
            const parsedComment =
              typeof change.comment === "string"
                ? JSON.parse(change.comment)
                : change.comment;
        ...
        const auditEntry = new AuditLog({
          timestamp: change.clusterTime
            ? new Date(change.clusterTime.high_water_mark * 1000)
            : new Date(),
          collectionName,
          operationType,
          documentId,
        ...
        console.error(
          "[AuditLogger Error] Failed to log database mutation event:",
          err.message,
        );
      }
    });
    ...
    console.log(
      "[AuditLogger] Change Stream listener initialized successfully.",
    );
  } catch (error) {
    console.error(
      "[AuditLogger] Failed to start Change Stream listener:",
      error.message,
    );
  }
};
