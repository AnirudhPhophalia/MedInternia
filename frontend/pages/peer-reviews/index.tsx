...
export default function PeerReviews() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/peer-reviews/user/${userId}/received`, {
  ...
  const fetchAnalytics = () => {
    ...
    api
      .get(`/peer-reviews/user/${userId}/analytics`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
  ...
  const renderTrendChart = (trend: {
    [key: string]: { received: number; given: number };
  }) => {
      ...
      <Box sx={{ width: "100%", overflowX: "auto", py: 2 }}>
        ...
        >
          ...
          {months.map((month, idx) => {
            ...
            const hReceived =
              (trend[month].received / maxVal) * (chartHeight - padding * 2);
            const hGiven =
              (trend[month].given / maxVal) * (chartHeight - padding * 2);
        ...
        {activeTab === 0 && (
          <Stack spacing={2}>
            {reviews.length === 0 ? (
              ...
              reviews.map((r) => <PeerReviewCard key={r._id} review={r} />)
            )}
          </Stack>
        )}
                ...
                <Grid container spacing={3}>
                  <Grid size={{ xs: 12, sm: 4 }}>
                    <Card
                      sx={{
                        background:
                          "linear-gradient(135deg, #1e3a8a 30%, #3b82f6 90%)",
                        color: "#fff",
                        borderRadius: 4,
                  ...
                  <Grid size={{ xs: 12, sm: 4 }}>
                    <Card
                      sx={{
                        background:
                          "linear-gradient(135deg, #6d28d9 30%, #a78bfa 90%)",
                        color: "#fff",
                        borderRadius: 4,
                  ...
                  <Grid size={{ xs: 12, sm: 4 }}>
                    <Card
                      sx={{
                        background:
                          "linear-gradient(135deg, #059669 30%, #34d399 90%)",
                        color: "#fff",
                        borderRadius: 4,
                      ...
                      <CardContent>
                        ...
                        >
                          {Object.keys(analytics.topTags).length === 0 ? (
                            ...
                            Object.entries(analytics.topTags).map(
                              ([tag, count]) => (
                                <Chip
                                  key={tag}
                                  label={`${tag} (${count})`}
