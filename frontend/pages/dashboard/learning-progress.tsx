    ...
    const fetchLearningData = async () => {
      try {
        ...
        try {
          ...
          setWebinars(
            webinarsRes.data?.data?.webinars ||
              webinarsRes.data?.webinars ||
              [],
          );
        } catch (e) {
        ...
        try {
          const starredIds = JSON.parse(
            localStorage.getItem("starredCases") || "[]",
          );
          if (starredIds.length > 0) {
            const casesRes = await api.get("/cases");
            const allCases =
              casesRes.data?.data?.cases || casesRes.data?.cases || [];
            const filtered = allCases.filter((c: any) =>
              starredIds.includes(c._id),
            );
              ...
              >
                <CardContent sx={{ p: 3 }}>
                  ...
                  <Typography variant="body2" sx={{ opacity: 0.9 }}>
                    Longest Streak:{" "}
                    <strong>{user.longestStreak || 0} days</strong>. Keep
                    studying daily to maintain your knowledge base!
                  </Typography>
                </CardContent>
              </Card>
                ...
                <CardContent sx={{ p: 3 }}>
                  ...
                  <Stack spacing={2}>
                    <Stack direction="row" alignItems="center" spacing={2}>
                      <Avatar
                        sx={{ bgcolor: "#fffbeb", border: "1px solid #fde68a" }}
                      >
                      ...
                      <Box>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {user.badgesEarned || 0} Badges total
                        </Typography>
                      </Box>
                    </Stack>
                    <Stack direction="row" alignItems="center" spacing={2}>
                      <Avatar
                        sx={{ bgcolor: "#ecfdf5", border: "1px solid #a7f3d0" }}
                      >
                      ...
                      <Box>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {user.certificatesEarned || 0} Certificates total
                        </Typography>
                      </Box>
                    </Stack>
                  </Stack>
                ...
                <CardContent sx={{ p: 3 }}>
                  ...
                  {savedCases.length === 0 ? (
                    <Alert severity="info" sx={{ borderRadius: 3 }}>
                      No saved cases. Go to the cases list and click the star
                      icon to save cases here for study later!
                    </Alert>
                  ) : (
                    <Grid container spacing={3}>
                      {savedCases.map((c) => (
