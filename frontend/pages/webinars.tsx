...
const getWebinarEndTime = (webinar: any) => {
  ...
  return new Date(
    new Date(webinar.scheduledAt).getTime() + duration * 60 * 1000,
  );
};
...
export default function WebinarsPage() {
  ...
  const fetchWebinars = () => {
    ...
    api
      .get(`/webinars?${params.toString()}`)
      .then((res) => {
        const fetchedWebinars = res.data.data.webinars || [];
        let visibleWebinars = fetchedWebinars;
        if (activeTab === "active") {
          visibleWebinars = fetchedWebinars.filter(
            (w: any) => !isWebinarExpired(w),
          );
        } else if (activeTab === "completed") {
          visibleWebinars = fetchedWebinars.filter((w: any) =>
            isWebinarExpired(w),
          );
        }
  ...
  if (selectedWebinar) {
    return (
      <WebinarJoin
        ...
        onLeave={() => setSelectedWebinar(null)}
      />
    );
  }
          ...
          <Grid size={{ xs: 12, sm: 4 }}>
            <FormControl fullWidth>
              ...
              >
                ...
                <MenuItem value="most_registered">
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title={
            activeTab === "active"
              ? "No active webinars"
              : "No completed webinars"
          }
          description={
            activeTab === "active"
              ? "Upcoming expert sessions will appear here once they are scheduled."
              : "Completed webinar recordings and history will appear here."
          }
          ...
          <Calendar
            ...
            events={webinars.map((w: any) => ({
              ...
              start: new Date(w.scheduledAt),
                ...
                new Date(w.scheduledAt).getTime() +
                  (w.duration || 60) * 60 * 1000,
              ),
              resource: w,
            }))}
            startAccessor="start"
        ...
        <Grid container spacing={3}>
          {webinars.map((w) => {
            ...
            const canJoin =
              activeTab === "active" && !expired && w.status === "live";
            const canRegister =
              activeTab === "active" &&
              !expired &&
              w.status === "scheduled" &&
                  ...
                  <CardContent sx={{ p: 3 }}>
                    ...
                    >
                      ...
                      <Stack direction="row" spacing={1}>
                        ...
                        <Chip
                          label={
                            expired ? "Completed" : w.status?.toUpperCase()
                          }
                    ...
                    <Stack spacing={1.5} sx={{ mb: 3 }}>
                      ...
                      >
                        ...
                        <Typography variant="caption">
                          {new Date(w.scheduledAt).toLocaleDateString()} at{" "}
                          {new Date(w.scheduledAt).toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                        </Typography>
