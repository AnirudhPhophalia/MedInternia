  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      setFeedback({
        isCorrect: false,
        message:
          error.response?.data?.message || "Incorrect answer. Try again!",
      });
    } finally {
      setSubmitting(false);
    }
          ...
          <CardContent sx={{ p: { xs: 3, md: 5 } }}>
            ...
            {step.type === "case" && (
              <Box>
                {step.caseRef ? (
                  <Box
                    sx={{ p: 3, bgcolor: "#f8fafc", borderRadius: 2, mb: 4 }}
                  >
                    ...
                    <Typography
                      ...
                      sx={{ mb: 2 }}
                    >
                      {step.caseRef.description}
                    </Typography>
                    <Button
                      variant="outlined"
                      onClick={() =>
                        window.open(`/cases/${step.caseRef._id}`, "_blank")
                      }
                    >
                    ...
                    {step.quiz.options.map((option: string, i: number) => (
                      <Box
                        ...
                        sx={{
                          ...
                          border: "1px solid",
                          borderColor:
                            selectedAnswer === i ? "primary.main" : "#e2e8f0",
                          bgcolor:
                            selectedAnswer === i
                              ? "rgba(0,114,255,0.05)"
                              : "transparent",
