...
export default function MentorshipDetail() {
  ...
  const handleToggleGoal = async (goalId: string) => {
    try {
      const token = localStorage.getItem("token");
      await api.patch(
        `/mentorship/${id}/goals/${goalId}/toggle`,
        {},
        { headers: { Authorization: `Bearer ${token}` } },
      );
      setMentorship((prev: any) => ({
        ...
        goals: prev.goals.map((g: any) =>
          g._id === goalId ? { ...g, isCompleted: !g.isCompleted } : g,
        ),
      }));
    } catch (err) {
      console.error(err);
                ...
                {mentorship.goals.length === 0 ? (
                  ...
                  <List>
                    {mentorship.goals.map((goal: any) => (
                      <ListItem
                        key={goal._id}
                        sx={{ bgcolor: "#f4f6f8", borderRadius: 2, mb: 1 }}
                      >
                        <ListItemIcon>
                          <Checkbox
                            edge="start"
                        ...
                        <ListItemText
                          ...
                          sx={{
                            textDecoration: goal.isCompleted
                              ? "line-through"
                              : "none",
