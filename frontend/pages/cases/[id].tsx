...
export default function CaseDiscussion({
  ...
  const canModerate =
    currentUser &&
    ["admin", "doctor", "moderator"].includes(currentUser.userType);
  ...
  useEffect(() => {
    ...
    if (token) {
      api
        ...
        .then((res) => {
          ...
          if (user.solvedCases) {
            setIsSolved(
              user.solvedCases.some(
                (scId: string) => scId.toString() === id.toString(),
              ),
            );
          }
        })
    ...
    api
      ...
      .then((res) => {
        ...
        setIsLiked(
          userId
            ? likes.some((likeId: any) => likeId.toString() === userId)
            : false,
        );
        setLoading(false);
      })
  ...
  const handleLike = async (commentId: string) => {
    try {
      const token = localStorage.getItem("token");
      await api.post(
        `/cases/${id}/comments/${commentId}/like`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleRate = async (commentId: string, rating: number) => {
    try {
      const token = localStorage.getItem("token");
      await api.post(
        `/cases/${id}/comments/${commentId}/rate`,
        { rating },
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleDiscussion = async () => {
    try {
      ...
      if (replyTo) {
        await api.post(
          `/cases/${id}/comments/${replyTo._id}/reply`,
  ...
  const handlePin = async (commentId: string) => {
    try {
      const token = localStorage.getItem("token");
      await api.post(
        `/cases/${id}/comments/${commentId}/pin`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleUnpin = async (commentId: string) => {
    try {
      const token = localStorage.getItem("token");
      await api.post(
        `/cases/${id}/comments/${commentId}/unpin`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const discussionPanel = (
    ...
    >
      ...
      {/* Main comments feed */}
      <Box
        sx={{ flex: 1, overflowY: "auto", maxHeight: "500px", pr: 1, mb: 2 }}
      >
                    ...
                    >
                      {discussions
                        ...
                        .map((r: any, rIdx: number) => {
                          const replyAuthorName = r.author
          ...
          <Box sx={{ mb: 3 }}>
            ...
            >
              <Typography
                ...
                fontWeight={900}
                color="text.primary"
                sx={{ flex: 1, letterSpacing: -0.5, mb: 0 }}
              >
                {caseData.title}
              </Typography>
              <Box sx={{ display: "flex", gap: 1 }}>
                <BookmarkButton
                  itemType="case"
                  itemId={caseData._id || (id as string)}
                />
                <Button
                  variant="outlined"
                  size="small"
                ...
                <OfflineSaveButton
                  caseId={caseData._id || (id as string)}
                  caseData={caseData}
                />
                <PdfExportButton
                  caseData={caseData}
              ...
              {caseData.patientInfo &&
                (caseData.patientInfo.age ||
                  ...
                  (caseData.patientInfo.medicalHistory &&
                    caseData.patientInfo.medicalHistory.length > 0) ||
                  (caseData.patientInfo.currentMedications &&
                    caseData.patientInfo.currentMedications.length > 0)) && (
                  <Card
                    sx={{
                      p: 2.5,
                      mb: 4,
                        ...
                        caseData.patientInfo.medicalHistory.length > 0 && (
                          <Grid size={{ xs: 12, sm: 6 }}>
                            ...
                            <Typography
                              ...
                              color="text.primary"
                            >
                              {caseData.patientInfo.medicalHistory.join(", ")}
                            </Typography>
                          </Grid>
                        )}
                      {caseData.patientInfo.currentMedications &&
                        caseData.patientInfo.currentMedications.length > 0 && (
                          <Grid size={{ xs: 12 }}>
                            ...
                            <Typography
                              ...
                              color="text.primary"
                            >
                              {caseData.patientInfo.currentMedications.join(
                                ", ",
                              )}
                            </Typography>
                          </Grid>
                ...
                <Box sx={{ mb: 4 }}>
                  ...
                  <Grid container spacing={2}>
                    {caseData.attachments.map((att: any, idx: number) => (
                      <Grid
                        size={{ xs: 12, sm: att.type === "audio" ? 12 : 6 }}
                        key={`att-${idx}`}
                      >
                        <Card
                          sx={{
