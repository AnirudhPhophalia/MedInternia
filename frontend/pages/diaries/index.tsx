...
const DiariesPage: React.FC = () => {
  ...
  const [newEntryDiseaseDescription, setNewEntryDiseaseDescription] =
    useState("");
  ...
  const [newEntrySymptomsChecklist, setNewEntrySymptomsChecklist] = useState<
    string[]
  >([]);
  ...
  const handleAddEntry = async () => {
    ...
    try {
      ...
      const normalizedDiary = {
        ...
        entries: (updatedDiary.entries || []).map((e: any) => ({
          ...e,
          id: e.id || e._id,
        })),
      };
      setDiaries(
        diaries.map((d) => (d.id === selectedDiary.id ? normalizedDiary : d)),
      );
...
`}</style>
      ...
      >
        <div
          ...
          style={{ width: "50%", minWidth: 340, maxWidth: 700 }}
        >
          <div
            style={{
              display: "flex",
          ...
          >
            <button
              className="med-btn"
              style={{ padding: "10px 32px", fontSize: 18, letterSpacing: 1 }}
          ...
          >
            <FaBookOpen className="med-accent" style={{ color: "#0072ff" }} />{" "}
            Daily Diaries
          </h1>
          {/* Create Diary Popup */}
          {showCreateDiary && (
              ...
              >
                ...
                >
                  <button
                    ...
                    className="med-btn"
                    style={{ fontSize: 17, padding: "10px 32px" }}
                  >
                    Create
                  </button>
                  <button
            ...
            >
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 13, color: "#888", marginTop: 2 }}>
                  Stats: Diaries: {diaries.length}, Days Logged:{" "}
                  {diaries.reduce((a, d) => a + d.entries.length, 0)}, Avg
                  Entries/Week:{" "}
                  {Math.round(
                    diaries.reduce((a, d) => a + d.entries.length, 0) / 2,
                  )}
                </div>
              </div>
              <div
          ...
          >
            <input
              type="text"
              placeholder="Search diaries, tags, symptoms..."
              value={searchTerm}
            ...
            <button className="med-btn" type="button">
              <svg width="18" height="18" fill="none" viewBox="0 0 24 24">
                <path
                  fill="#fff"
                  d="M5 20h14v-2H5v2zm7-18C6.48 2 2 6.48 2 12c0 5.52 4.48 10 10 10s10-4.48 10-10c0-5.52-4.48-10-10-10zm1 15h-2v-6H8l4-4 4 4h-3v6z"
                />
              </svg>{" "}
              Export PDF
            </button>
          ...
          <h2
            ...
            className="med-blue"
          >
            <FaBookOpen style={{ color: "#3fdde6ff" }} /> Diaries Timeline
          </h2>
          {diaries.map((diary) => (
            <div
              key={diary.id}
          ...
          {selectedEntry && (
            ...
            >
              ...
              >
                ...
                >
                  Day {selectedEntry.day} Details
                </h3>
                <div style={{ color: "#333", fontSize: 15 }}>
                  <b>Time:</b> {selectedEntry.time}
                </div>
                <div style={{ color: "#333", fontSize: 15 }}>
                  <b>Location:</b> {selectedEntry.location}
                </div>
                <div style={{ color: "#333", fontSize: 15 }}>
                  <b>Disease Description:</b> {selectedEntry.diseaseDescription}
                </div>
                <div style={{ color: "#333", fontSize: 15 }}>
                  <b>Symptoms:</b> {selectedEntry.symptoms}
                </div>
                <div style={{ color: "#333", fontSize: 15 }}>
                  <b>Tags:</b> {selectedEntry.tags?.join(", ")}
                </div>
                <div style={{ color: "#333", fontSize: 15 }}>
                  <b>Doctor Reference:</b> {selectedEntry.doctorReference}
                </div>
                <div style={{ color: "#333", fontSize: 15 }}>
                  <b>Gender:</b> {selectedEntry.gender}
                </div>
                <div style={{ color: "#333", fontSize: 15 }}>
                  <b>Data Source:</b> {selectedEntry.dataSource}
                </div>
                <div style={{ color: "#333", fontSize: 15 }}>
                  <b>Symptom Checklist:</b>{" "}
                  {selectedEntry.symptomsChecklist?.join(", ")}
                </div>
                ...
                <div style={{ marginTop: 8, color: "#333", fontSize: 15 }}>
                  <b>Notes:</b> {selectedEntry.content}
                </div>
                ...
                <div style={{ marginTop: 8 }}>
                  <b>Likes:</b> {selectedEntry.likes || 0} <b>Comments:</b>{" "}
                  {selectedEntry.comments?.length || 0}
                  <div style={{ marginTop: 4 }}>
                    {selectedEntry.comments?.map((c, i) => (
                      <div
                        key={i}
                        style={{ fontSize: 13, color: "#555", marginBottom: 2 }}
                      >
                        <b>{c.user}:</b> {c.text}
                      </div>
                    ))}
              ...
              >
                ...
                <div style={{ width: "100%" }}>
                  ...
                  <input
                    ...
                    value={newEntryDiseaseDescription}
                    onChange={(e) =>
                      setNewEntryDiseaseDescription(e.target.value)
                    }
                    style={{
                      width: "100%",
                      marginBottom: 12,
                  ...
                  <input
                    ...
                    value={newEntrySymptoms}
                    onChange={(e) => setNewEntrySymptoms(e.target.value)}
                    style={{
                      width: "100%",
                      marginBottom: 12,
                      padding: 10,
                  ...
                  <input
                    ...
                    value={newEntryDoctorReference}
                    onChange={(e) => setNewEntryDoctorReference(e.target.value)}
                    style={{
                      width: "100%",
                      marginBottom: 12,
                      padding: 10,
                  ...
                  <input
                    type="text"
                    placeholder="Data Source (Hospital, NGO, etc.)"
                    value={newEntryDataSource}
                    onChange={(e) => setNewEntryDataSource(e.target.value)}
                    style={{
                      width: "100%",
                      marginBottom: 12,
                      padding: 10,
                  ...
                  <input
                    type="text"
                    placeholder="Gender (Male, Female, Other, Custom)"
                    value={newEntryGender}
                    onChange={(e) => setNewEntryGender(e.target.value)}
                    style={{
                      width: "100%",
                      marginBottom: 12,
                      padding: 10,
