...
function DoctorMentorshipCard({
  doctor,
  currentUserId,
  currentMentorId,
  onApplyMentorship,
  userRole,
}: DoctorMentorshipCardProps) {
  const [summary, setSummary] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api
      .get(`/users/${doctor._id}/mentor-summary`)
      .then((res) => {
        setSummary(
          res.data?.data?.mentorStats || res.data?.mentorStats || null,
        );
      })
      .catch(() => {})
    ...
    >
      <CardContent sx={{ p: 3 }}>
        ...
        >
          <Box>
            ...
            <Typography variant="caption" color="text.secondary">
              Specialization:{" "}
              <strong>{doctor.specialization || "General"}</strong>
            </Typography>
          </Box>
        ...
        {loading ? (
          ...
          <Alert severity="info" sx={{ py: 0.5, px: 1, mb: 3 }}>
            No mentor summary stats found.
          </Alert>
        )}
...
function Doctors() {
  ...
  const handleApplyMentorship = async (doctorId: string) => {
    try {
      ...
      const msg =
        err.response?.data?.message || "Failed to apply for mentorship.";
      alert(msg);
    }
  };
      ...
      <Container maxWidth="lg">
        ...
        <Box sx={{ mb: 4, textAlign: "center" }}>
          ...
          <Typography variant="subtitle1" color="text.secondary">
            Find clinical mentors, track mentoring achievements, and manage
        ...
        {isDoctor && (
          ...
          >
            ...
            <Tab
              label={`My Mentees (${mentees.length})`}
              sx={{ fontWeight: 600 }}
            />
          </Tabs>
        )}
              ...
              <form onSubmit={handleSmartSearch}>
                <Stack spacing={2}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    ...
                    automatically filter by specialization, location,
                    experience, and name.
                  </Typography>
                  ...
                  {smartSearchActive && (
                    ...
                    >
                      <Typography
                        ...
                        color="primary"
                      >
                        Showing {doctors.length} smart search result
                        {doctors.length !== 1 ? "s" : ""}
                      </Typography>
                      <Button
                        size="small"
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              {mentees.length === 0 ? (
                <Alert severity="info" sx={{ borderRadius: 3 }}>
                  No interns have chosen you as their mentor yet. Let interns
                  know they can apply for mentorship on their doctors directory
                  tab!
                </Alert>
              ) : (
                <TableContainer
                  ...
                  <Table aria-label="mentees table">
                    <TableHead sx={{ bgcolor: "#f8fafc" }}>
                      <TableRow>
                        ...
                        <TableCell>
                          <strong>Medical School / Year</strong>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableRow key={m._id}>
                          ...
                          <TableCell>
                            <Typography variant="body2">
                              {m.medicalSchool || "Unspecified"}
                            </Typography>
                            <Typography
                              variant="caption"
                              color="text.secondary"
                            >
                              Year {m.yearOfStudy || "N/A"}
                            </Typography>
                          </TableCell>
                          <TableCell>
                            <Chip
                              label={`${m.points} Points`}
                              size="small"
                              color="primary"
                              sx={{ fontWeight: 700 }}
                            />
