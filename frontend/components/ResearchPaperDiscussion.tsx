...
export default function ResearchPaperDiscussion({
  id,
  modalMode,
}: {
  id: string;
  ...
  const handleLike = async (commentId: string) => {
    try {
      const token = localStorage.getItem("token");
      await api.post(
        `/research-papers/${id}/comments/${commentId}/like`,
        {},
        {
  ...
  const handleDiscussion = async () => {
    try {
      ...
      if (replyTo) {
        await api.post(
          `/research-papers/${id}/comments/${replyTo._id}/reply`,
            ...
            return (
              ...
              >
                <Box
                  sx={{
                    background: isMe
                      ? "linear-gradient(135deg, #1976d2 60%, #64b5f6 100%)"
                      : "linear-gradient(135deg, #90caf9 60%, #e3f2fd 100%)",
                    color: "#fff",
                    width: 40,
                    height: 40,
                    borderRadius: "50%",
                    ...
                    <Tooltip title="Like">
                      ...
                      >
                        <ThumbUpAltOutlinedIcon
                          sx={{
                            fontSize: 18,
                            color: c.likedBy?.includes(userId)
                              ? "#1976d2"
                              : "#0072ff",
                          }}
                        />
