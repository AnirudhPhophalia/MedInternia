...
export default function OfflineSaveButton({
  ...
  const handleSaveOffline = async () => {
    if (!("caches" in window)) {
      setSnack({
        open: true,
        message: "Offline mode is not supported in this browser.",
        severity: "error",
      });
      return;
    }
    ...
    try {
      ...
      const apiResponse = new Response(
        JSON.stringify({ success: true, data: { case: caseData } }),
        {
      ...
      setSnack({
        open: true,
        message: "Case saved for offline viewing!",
        severity: "success",
      });
    } catch (error) {
  ...
  return (
    <>
      <Tooltip
        title={isSaved ? "Saved Offline" : "Download for Offline Viewing"}
      >
        <span>
          <Button
