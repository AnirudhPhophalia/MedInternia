  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const drawSectionHeader = (
        label: string,
        y: number,
        bgColor: [number, number, number] = [0, 86, 204],
      ): number => {
      ...
      yPos = addText(fullName, margin, yPos, {
        fontSize: 24,
        fontStyle: "bold",
        color: [0, 86, 204],
      });
      ...
      const roleText =
        user.userType === "doctor" && user.specialization
          ...
          : user.userType === "patient"
            ? "Patient Profile"
            : "Medical Intern";
      yPos = addText(roleText, margin, yPos, {
        fontSize: 14,
        fontStyle: "italic",
        color: [100, 100, 100],
      });
      ...
      yPos = addText(contactInfo, margin, yPos, {
        fontSize: 10,
        color: [80, 80, 80],
      });
      ...
      if (user.medicalSchool) {
        yPos = addText(`Medical School: ${user.medicalSchool}`, margin, yPos, {
          fontStyle: "bold",
        });
      }
      if (user.yearOfStudy) {
      ...
      if (user.experience) {
        yPos = addText(`Experience: ${user.experience} Years`, margin, yPos, {
          fontStyle: "bold",
        });
      }
      ...
      if (user.interests && user.interests.length > 0) {
        yPos = addText(
          `Clinical Interests: ${user.interests.join(", ")}`,
          margin,
          yPos,
        );
      }
      ...
      yPos = addText(`Platform Points: ${user.points || 0}`, margin, yPos, {
        fontStyle: "bold",
        color: [0, 114, 255],
      });
      yPos = addText(
        `Cases Analyzed: ${user.casesAnalyzed || 0}`,
        margin,
        yPos,
      );
      yPos = addText(
        `Peer Reviews Given: ${user.peerReviewsGiven || 0}`,
        margin,
        yPos,
      );
      yPos = addText(
        `Certificates Earned: ${user.certificatesEarned || 0}`,
        margin,
        yPos,
      );
      if (user.streak) {
        yPos = addText(
          `Current Activity Streak: ${user.streak} days`,
          margin,
          yPos,
        );
      }
      ...
      if (badges && badges.length > 0) {
        ...
        badges.forEach((b: any) => {
          ...
          if (badgeDesc) {
            yPos = addText(`  ${badgeDesc}`, margin, yPos, {
              fontSize: 10,
              color: [100, 100, 100],
            });
          }
